...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        # subject = '天天生鲜欢迎信息'
        # message = ''
        # html_message = f'<h1>{username},欢迎你成为天天生鲜会员,请点击下面链接激活账号</h1><br/><a href="http://127.0.0.1:8000/user/actice/{token}">http://127.0.0.1:8000/user/actice/{token}</a>'
...
class UserInfoView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        # sr = StrictRedis(host='127.0.0.1', db=9)
        ...
        for id in sku_ids:
            ...
            goods.image = goods.goodsimage_set.all().first().image
    ...
    def get(self, request, page):
        ...
        for order in orders:
            ...
            for order_sku in order_skus:
                ...
                order_sku.image = order_sku.sku.goodsimage_set.all().first().image
            # 动态给order增加属性，保存订单状态标题
            # order.status_name = OrderInfo.ORDER_STATUS[order.order_status]
            # 动态给order增加属性，保存订单商品的信息
            order.order_skus = order_skus
            order.pay_status = order.ORDER_STATUS[order.order_status]
            # print(order_skus)
